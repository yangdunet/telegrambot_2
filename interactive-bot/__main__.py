...
from .message_delete import delete_message, delete_recent_messages
    ...
    buttons.append(
        [
            InlineKeyboardButton(
                ...
                url=f"https://github.com/YanSi/Telegram-interactive-bot",
            )
        ]
    )
    if user.username:
...
async def clear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user.id in admin_user_ids:
        await update.message.reply_html("你没权限执行此操作。")
        return
...
async def broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    if not user.id in admin_user_ids:
        await update.message.reply_html("你没有权限执���此操作。")
        return
...
def main():
    ...
    application.add_handler(CommandHandler("del", lambda update, context: delete_message(update, context, db)))
    application.add_handler(CommandHandler("delrecent", lambda update, context: delete_recent_messages(update, context, db)))
